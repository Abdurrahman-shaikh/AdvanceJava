 ...
 * @author azmi
 */
@WebServlet(name = "JDServlet", urlPatterns = {"/JDServlet"})
public class JDServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String query = "SELECT id,name, city FROM friends";
            ResultSet rs = (ResultSet)st.executeQuery(query);
            out.println("<ul>");
            while (rs.next()) {
                out.println("<li><a href=FriendServlet?id=" + rs.getString("id") + ">" + rs.getString("name") + "</a></li>");
            }
            out.println("</ul>");
